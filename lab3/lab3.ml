...
let score_mem (xs: string) (ys: string) =
  let pts_space = -2 in
  let x_len = String.length xs in
  let y_len = String.length ys in
  ...
  let revise
    ...
    (pts + score, String.make 1 x_char ^ xs, String.make 1 y_char ^ ys)
  in let rec aux (xi: int) (yi: int) =
    try Hashtbl.find ht (xi, yi)
    with
    | _ ->
        if xi = x_len && yi == y_len then
          let result = (0, "", "") in
          Hashtbl.add ht (xi, yi) result;
          result;
        else if (yi == y_len) then
          let num = x_len - xi in
          let pts = pts_space * num in
          ...
          let m = mt x_char y_char in
          let r3 = revise (aux (xi + 1) (yi + 1)) m x_char y_char in
          let result = max r1 (max r2 r3) in
          Hashtbl.add ht (xi, yi) result;
          result;
  in aux 0 0;;

let ans1 = score_mem "A" "A";;
let ans2 = score_mem "A" "AB";;
let ans3 = score_mem "GATCGGCAT" "CAATGTGAATC";;
